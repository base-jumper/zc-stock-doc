---
name: "stock-analysis"
description: "Prevent skipped TAM models with sticky components, dependency resolution, and a valuation completion gate."
status: proposal
version: "v1"
date: "2026-08-01T06:28:26.016Z"
---
...
* Each strategy's front matter carries a `valuation:` field naming its method — wonderful-and-fair and cash-cow both use
  ...
  step for it.
* **TAM-capture has a market-doc prerequisite.** Decide whether TAM-capture applies from the
  business and thesis **before** checking whether its dependency exists. Dependency availability
  never determines applicability. Follow the dependency workflow in the
  ...
  pass that document's reference into `valuation.tam-capture.market-doc`. A missing, legacy, or
  invalid market input is a dependency to resolve—not permission to omit TAM-capture, reduce its
  weight to zero, or publish a partial blend.
* **Blended valuations** (`weighted-average`) name their components as the keys of a `weights` map and
  must run *after* them. Determine the applicable component set first, resolve every component's
  dependencies, run each component, then run the meta-method. During a refresh, read and preserve the
  existing `weights` keys before editing: an existing component is presumed applicable unless new
  business or thesis evidence makes it genuinely inapplicable. Document that evidence before removing
  it. A schema failure or missing prerequisite is not an applicability decision. The meta-method reads
...
re-research), first freeze the current method set: read the whole `valuation:` block and, for a
`weighted-average`, record its existing `weights` keys before editing. Refresh every listed
component and preserve the set unless new business or thesis evidence changes applicability. Update
inputs that moved—most often `price`, but also growth, fair value, or a catalyst's
probability/window—then rerun each component script and the meta-method.

Treat a legacy schema, missing dependency, or invalid input as migration work, not a reason to delete
the component or publish a one-component substitute. For applicable TAM-capture, complete the
market-doc dependency workflow even when the request sounds lightweight. Update the Valuation section
to match the assumptions, ROI, and attribution, then run the overall-score step.

**Valuation completion gate.** Do not update the canonical ROI, overall scores, or `last-updated`
until all of these are true:

1. Applicable methods were decided from the business and thesis, not from available inputs.
2. Every dependency exists and validates; legacy inputs were migrated rather than silently dropped.
3. Every applicable component was rerun using the same as-of date.
4. The weighted-average script ran after its components, and the overall-score script ran last.
5. The Valuation section names every included component and explains any applicability-driven removal.
